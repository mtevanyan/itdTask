var slider = function (sliderElement) {
    ...
    var init = function () {
        ...
        var vsNextArrow = document.createElement('button');
        vsNextArrow.classList.add('vsArrow');
        vsNextArrow.id = 'vsNextArrow';
        vsNextArrow.insertAdjacentHTML('beforeend', `&#8595`);
        indicatorContainer.appendChild(vsNextArrow);

        var vsPrevArrow = document.createElement('button');
        vsPrevArrow.classList.add('vsArrow');
        vsPrevArrow.id = 'vsPrevArrow';
        vsPrevArrow.insertAdjacentHTML('beforeend', `&#8593`);
        indicatorContainer.appendChild(vsPrevArrow);
        ...
        document.getElementById('vsNextArrow').onclick = function () {
            changeSlide(1);
        }

        document.getElementById('vsPrevArrow').onclick = function () {
            changeSlide(-1);
        }
    ...
    var changeSlide = function (direction) {
        ...
        // change page
        if (currentSlide == 2) {
            document.querySelector(".slider__indicators").classList.add('dotes-white');
        } else {
            // if ( document.querySelector(".slider__indicators").classList.contains('dotes-white') ) {
            document.querySelector(".slider__indicators").classList.remove('dotes-white');
            // }
        }

        if (pages.length == currentSlide) {
            document.getElementById("vsNextArrow").style.visibility = 'hidden';
        } else {
            document.getElementById("vsNextArrow").style.visibility = 'visible';
        }


        if (currentSlide > 1) {
            document.getElementById("vsPrevArrow").style.visibility = 'visible';
        } else {
            document.getElementById("vsPrevArrow").style.visibility = 'hidden';
        }
        isChanging = true;
